...
def init_logger(round_start_time):
    ...
    log_path = os.path.join(log_dir, "UM_Debug.log")
    if os.path.exists(log_path):
        try: os.remove(log_path)
        except: pass

    logger = logging.getLogger('UMBot')
    ...
    logger.info("=" * 60)
    logger.info("UM PAYMENT GATEWAY TEST STARTING...")
...
async def perform_login(page):
    WEBSITE_URL = "https://www.uea8.com/en-my"
    for _ in range(3):
        try:
            log.info(f"LOGIN PROCESS - OPENING WEBSITE: {WEBSITE_URL}")
            await page.goto("https://www.uea8.com/en-my", timeout=30000, wait_until="domcontentloaded")
    ...
    try:
        ...
        await username_password_login_container.locator('input.undefined.standard-input[data-name="username"]').fill("bottestingss")
...
async def url_jump_check(page,new_page,context,old_url,deposit_submit_button,deposit_option,deposit_method,deposit_channel,bank_name):
    ...
    if new_payment_page == True:
        ...
        while retry_count < max_retries:
            try:
                ...
                await new_page.screenshot(path="UMUEA8_%s_%s_%s-%s_Payment_Page.png"%(deposit_option,deposit_method,deposit_channel,bank_name),timeout=30000)
                break 
            except TimeoutError:
    ...
    if new_payment_page == False:   
        await page.screenshot(path="UMUEA8_%s_%s_%s-%s_Payment_Page.png"%(deposit_option,deposit_method,deposit_channel,bank_name),timeout=30000)
        url_jump = False
    ...
    try:
        for _ in range(20):
            ...
            if await toast.count() > 0:
                toast_exist = True
                await new_page.screenshot(path="UMUEA8_%s_%s_%s-%s_Payment_Page.png"%(deposit_option,deposit_method_text,deposit_channel,bank_name),timeout=30000)
...
async def perform_payment_gateway_test(page,context):
    ...
    try:
        ...
        for i in range(deposit_options_total_count):
            ...
            try:
                deposit_option = deposit_option.replace("/", "-")
            except Exception as e:
                log.info("PERFORM PAYMENT GATEWAY TEST - DEPOSIT OPTION [%s] TEXT NO / [%s] : %s"%(deposit_option,e))
...
URL: [uea8\\.com](https://www\\.uea8\\.com/en\\-my)
TEAM : UM
...
**Time Detail**  
├─ **TimeOccurred:** `{timestamp}` """ 
            files = glob.glob("*UMUEA8_%s_%s_%s*.png"%(deposit_option,deposit_method,deposit_channel))
...
async def clear_screenshot():
    picture_to_sent = glob.glob("*UMUEA8*.png")
