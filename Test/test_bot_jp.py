...
def init_logger(round_start_time):
    ...
    log_path = os.path.join(log_dir, "JP_Debug.log")
    if os.path.exists(log_path):
        try: os.remove(log_path)
        except: pass

    logger = logging.getLogger('JPBot')
    ...
    logger.info("=" * 60)
    logger.info("JP PAYMENT GATEWAY TEST STARTING...")
...
async def perform_login(page):
    WEBSITE_URL = "https://www.jolibetph6.com/en-ph/"
    for _ in range(3):
        try:
            log.info(f"LOGIN PROCESS - OPENING WEBSITE: {WEBSITE_URL}")
            await page.goto("https://www.jolibetph6.com/en-ph/", timeout=30000, wait_until="domcontentloaded")
            await wait_for_network_stable(page, timeout=30000)
    ...
    try:
        login_button = page.locator('button.btn.primary:has-text("Login")')
    ...
    if new_payment_page == True:
        ...
        while retry_count < max_retries:
            try:
                ...
                await page.screenshot(path="JP_%s_%s-%s_Payment_Page.png"%(deposit_method,deposit_channel,bank_name),timeout=30000)
                break 
            except TimeoutError:
...
**Time Detail**  
├─ **TimeOccurred:** `{timestamp}` """
            files = glob.glob("*JP_%s_%s*.png"%(deposit_method,deposit_channel))
        ...
        try:
            if os.path.exists(file):
                ...
                if "JP" in sheets:
                    for attempt in range(3):
                        try:
                            df = pd.read_excel(file,sheet_name="JP")
                        except Exception as e:
                    ...
                    for attempt in range(3):
                        try:
                            with pd.ExcelWriter(
                                ...
                                df.to_excel(writer, sheet_name='JP', index=False)
                        except Exception as e:
                            log.warning(f"DATA PROCESS EXCEL ERROR: {e}，RETRY {attempt + 1}/3...")
                            await asyncio.sleep(5)
                else:
                    log.info("Sheets JP not found in file :%s"%file)
