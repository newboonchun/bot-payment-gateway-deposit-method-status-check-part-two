...
def init_logger(round_start_time):
    ...
    log_path = os.path.join(log_dir, "I8N_Debug.log")
    if os.path.exists(log_path):
        try: os.remove(log_path)
        except: pass

    logger = logging.getLogger('I8NBot')
    ...
    logger.info("=" * 60)
    logger.info("I8N PAYMENT GATEWAY TEST STARTING...")
...
async def perform_login(page):
    WEBSITE_URL = "https://www.ime88.com/en-np"
    for _ in range(3):
        try:
            log.info(f"LOGIN PROCESS - OPENING WEBSITE: {WEBSITE_URL}")
            await page.goto("https://www.ime88.com/en-np", timeout=30000, wait_until="domcontentloaded")
            await wait_for_network_stable(page, timeout=30000)
    ...
    if new_payment_page == True:
        ...
        while retry_count < max_retries:
            try:
                ...
                await page.screenshot(path="I8N_%s_%s-%s_Payment_Page.png"%(deposit_method,deposit_channel,bank_name),timeout=30000)
                break 
            except TimeoutError:
...
URL: [ime88\\.com](https://www\\.ime88\\.com/en\\-np)
TEAM : I8N
...
**Time Detail**  
├─ **TimeOccurred:** `{timestamp}` """
            files = glob.glob("*I8N_%s_%s*.png"%(deposit_method,deposit_channel))
...
async def clear_screenshot():
    picture_to_sent = glob.glob("*I8N*.png")
        ...
        try:
            if os.path.exists(file):
                ...
                if "I8N" in sheets:
                    for attempt in range(3):
                        try:
                            df = pd.read_excel(file,sheet_name="I8N")
                        except Exception as e:
                    ...
                    for attempt in range(3):
                        try:
                            with pd.ExcelWriter(
                                ...
                                df.to_excel(writer, sheet_name='I8N', index=False)
                        except Exception as e:
                            log.warning(f"DATA PROCESS EXCEL ERROR: {e}，RETRY {attempt + 1}/3...")
                            await asyncio.sleep(5)
                else:
                    log.info("Sheets I8N not found in file :%s"%file)
