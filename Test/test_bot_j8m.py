...
def init_logger(round_start_time):
    ...
    log_path = os.path.join(log_dir, "J8M_Debug.log")
    if os.path.exists(log_path):
        try: os.remove(log_path)
        except: pass

    logger = logging.getLogger('J8MBot')
    ...
    logger.info("=" * 60)
    logger.info("J8M PAYMENT GATEWAY TEST STARTING...")
...
async def perform_login(page):
    WEBSITE_URL = "https://www.jw8myr.com/en-my"
    for _ in range(3):
        try:
            log.info(f"LOGIN PROCESS - OPENING WEBSITE: {WEBSITE_URL}")
            await page.goto("https://www.jw8myr.com/en-my", timeout=30000, wait_until="domcontentloaded")
            await wait_for_network_stable(page, timeout=30000)
    ...
    await asyncio.sleep(1)
    try:
        await page.get_by_role("textbox", name="Username").fill("bottestinggg")
    ...
    if new_payment_page == True:
        ...
        while retry_count < max_retries:
            try:
                ...
                await page.screenshot(path="J8M_%s_%s-%s_Payment_Page.png"%(deposit_method,deposit_channel,bank_name),timeout=30000)
                break 
            except TimeoutError:
...
URL: [jw8myr\\.com](https://www\\.jw8myr\\.com/en\\-my)
TEAM : J8M
...
├─ **TimeOccurred:** `{timestamp}` """ 
            ...
            michael_caption = f"""[derrick0302](tg://user?id=1413625020), [JW8CSmyr](tg://user?id=6646318226), [Broai7788](tg://user?id=5782429875)
*Subject: Bot Testing Deposit Gateway*  
URL: [jw8myr\\.com](https://www\\.jw8myr\\.com/en\\-my)
TEAM : J8M
┌─ **Deposit Testing Result** ──────────┐
...
**Time Detail**  
├─ **TimeOccurred:** `{timestamp}` """
            files = glob.glob("*J8M_%s_%s*.png"%(deposit_method,deposit_channel))
...
async def clear_screenshot():
    picture_to_sent = glob.glob("*J8M*.png")
        ...
        try:
            if os.path.exists(file):
                ...
                if "J8M" in sheets:
                    for attempt in range(3):
                        try:
                            df = pd.read_excel(file,sheet_name="J8M")
                        except Exception as e:
                    ...
                    for attempt in range(3):
                        try:
                            with pd.ExcelWriter(
                                ...
                                df.to_excel(writer, sheet_name='J8M', index=False)
                        except Exception as e:
                            log.warning(f"DATA PROCESS EXCEL ERROR: {e}，RETRY {attempt + 1}/3...")
                            await asyncio.sleep(5)
                else:
                    log.info("Sheets J8M not found in file :%s"%file)
