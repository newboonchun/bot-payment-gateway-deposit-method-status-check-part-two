...
async def telegram_send_operation(telegram_message, failed_reason, program_complete):
    ...
    ck_chat_id = os.getenv("CK_CHAT_ID")
    bot = Bot(token=TOKEN)
...
├─ **TimeOccurred:** `{timestamp}` """ 
            ...
            ck_caption = f"""[becomereal66](tg://user?id=7532980241)
*Subject: Bot Testing Deposit Gateway*  
URL: [dis88\\.net](https://www\\.dis88\\.net/en\\-my)
TEAM : D8M
┌─ **Deposit Testing Result** ──────────┐
...
{summary_body}"""
    ...
    for attempt in range(3):
        try:
            await bot.send_message(chat_id=ck_chat_id, text=caption, parse_mode='MarkdownV2', disable_web_page_preview=True)
            log.info("SUMMARY SENT")
            break
        except TimedOut:
            log.warning(f"TELEGRAM TIMEOUT，RETRY {attempt + 1}/3...")
