...
def init_logger(round_start_time):
    ...
    log_path = os.path.join(log_dir, "J8PT_Debug.log")
    if os.path.exists(log_path):
        try: os.remove(log_path)
        except: pass

    logger = logging.getLogger('J8PTBot')
    ...
    logger.info("=" * 60)
    logger.info("J8PT PAYMENT GATEWAY TEST STARTING...")
...
async def perform_login(page):
    WEBSITE_URL = "https://www.jw8my.com/en-pk"
    for _ in range(3):
        try:
            log.info(f"LOGIN PROCESS - OPENING WEBSITE: {WEBSITE_URL}")
            await page.goto("https://www.jw8my.com/en-pk", timeout=30000, wait_until="domcontentloaded")
            await wait_for_network_stable(page, timeout=30000)
    ...
    if new_payment_page == True:
        ...
        while retry_count < max_retries:
            try:
                ...
                await page.screenshot(path="J8PT_%s_%s-%s_Payment_Page.png"%(deposit_method,deposit_channel,bank_name),timeout=30000)
                break 
            except TimeoutError:
...
async def check_toast(page,deposit_method,deposit_channel,bank_name):
    ...
    matches = re.findall(r"Rs\s*([\d,]+)", money_input_range_text)
    if matches:
...
URL: [jw8my\\.com](https://www\\.jw8my\\.com/en\\-pk)
TEAM : J8PT
...
**Time Detail**  
├─ **TimeOccurred:** `{timestamp}` """
            files = glob.glob("*J8PT_%s_%s*.png"%(deposit_method,deposit_channel))
...
async def clear_screenshot():
    picture_to_sent = glob.glob("*J8PT*.png")
        ...
        try:
            if os.path.exists(file):
                ...
                if "J8PT" in sheets:
                    for attempt in range(3):
                        try:
                            df = pd.read_excel(file,sheet_name="J8PT")
                        except Exception as e:
                    ...
                    for attempt in range(3):
                        try:
                            with pd.ExcelWriter(
                                ...
                                df.to_excel(writer, sheet_name='J8PT', index=False)
                        except Exception as e:
                            log.warning(f"DATA PROCESS EXCEL ERROR: {e}，RETRY {attempt + 1}/3...")
                            await asyncio.sleep(5)
                else:
                    log.info("Sheets J8PT not found in file :%s"%file)
