...
def init_logger(round_start_time):
    ...
    log_path = os.path.join(log_dir, "N8Y_Debug.log")
    if os.path.exists(log_path):
        try: os.remove(log_path)
        except: pass

    logger = logging.getLogger('N8YBot')
    ...
    logger.info("=" * 60)
    logger.info("N8Y PAYMENT GATEWAY TEST STARTING...")
...
async def reenter_deposit_page(page,old_url,deposit_method,deposit_channel,method_btn,channel_btn,bank_name,bank_btn,min_amount,recheck):
    ...
    try:
        ...
        #log.info("REENTER DEPOSIT PAGE - DEPOSIT METHOD [%s] BUTTON ARE FAILED CLICKED [%s]"%(deposit_method,e))
...
async def perform_login(page):
    WEBSITE_URL = "https://www.nex8.com/en-mm"
    for _ in range(3):
        try:
            log.info(f"LOGIN PROCESS - OPENING WEBSITE: {WEBSITE_URL}")
            await page.goto("https://www.nex8.com/en-mm", timeout=30000, wait_until="domcontentloaded")
            await wait_for_network_stable(page, timeout=30000)
    ...
    #<button data-v-9fa67586="" type="button" class="mission_daily_close_btn" aria-label="Close"><img data-v-9fa67586="" src="/img/icons/close.svg" alt="close"></button>
    try:
        ...
        log.info("LOGIN PROCESS - MISSION DAILY CLOSE BUTTON ARE NOT CLICKED")
...
async def url_jump_check(page,old_url,deposit_method,deposit_channel,bank_name,bank_btn,money_button_text,method_btn,channel_btn,telegram_message):
    try:
            ...
            try:
                ...
                if qr_code_count != 0:
                    ...
                    if retry_count == max_retries:
                        ...
                        try:
                            await reenter_deposit_page(page,old_url,deposit_method,deposit_channel,method_btn,channel_btn,bank_name,bank_btn,money_button_text,recheck=1)
                        except Exception as e:
...
async def check_toast(page,deposit_method,deposit_channel,bank_name):
    ...
    matches = re.findall(r"Ks.\s*([\d,]+)", money_input_range_text)
    if matches:
...
**Time Detail**  
├─ **TimeOccurred:** `{timestamp}` """
            files = glob.glob("*N8Y_%s_%s*.png"%(deposit_method,deposit_channel))
        ...
        try:
            if os.path.exists(file):
                ...
                if "N8Y" in sheets:
                    for attempt in range(3):
                        try:
                            df = pd.read_excel(file,sheet_name="N8Y")
                        except Exception as e:
                    ...
                    for attempt in range(3):
                        try:
                            with pd.ExcelWriter(
                                ...
                                df.to_excel(writer, sheet_name='N8Y', index=False)
                        except Exception as e:
                            log.warning(f"DATA PROCESS EXCEL ERROR: {e}，RETRY {attempt + 1}/3...")
                            await asyncio.sleep(5)
                else:
                    log.info("Sheets N8Y not found in file :%s"%file)
