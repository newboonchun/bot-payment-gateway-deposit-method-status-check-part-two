...
def init_logger(round_start_time):
    ...
    log_path = os.path.join(log_dir, "N9N_Debug.log")
    if os.path.exists(log_path):
        try: os.remove(log_path)
        except: pass

    logger = logging.getLogger('N9NBot')
    ...
    logger.info("=" * 60)
    logger.info("N9N PAYMENT GATEWAY TEST STARTING...")
...
async def perform_login(page):
    WEBSITE_URL = "https://www.nep9now.com/ne-np"
    for _ in range(3):
        try:
            log.info(f"LOGIN PROCESS - OPENING WEBSITE: {WEBSITE_URL}")
            await page.goto("https://www.nep9now.com/ne-np", timeout=30000, wait_until="domcontentloaded")
            await wait_for_network_stable(page, timeout=30000)
    ...
    try:
        login_button = page.locator('button.btn.primary.new-reg-buttons:has-text("लगइन")')
    ...
    #<button data-v-b75cb264="" type="button" class="mission_daily_close_btn" aria-label="बन्द गर्नुहोस्"><img data-v-b75cb264="" src="/img/icons/close.svg" alt="close"></button>
    try:
        mission_daily_close_button = page.locator("button.mission_daily_close_btn")
        await mission_daily_close_button.click()
        log.info("LOGIN PROCESS - MISSION DAILY CLOSE BUTTON ARE CLICKED")
    except:
        log.info("LOGIN PROCESS - MISSON DAILY CLOSE BUTTON ARE NOT CLICKED")
    ...
    try:
        ...
        deposit_topbar_button = deposit_topbar_container.locator('button.topbar_btn_2:has-text("जमा")')
...
async def url_jump_check(page,old_url,deposit_method,deposit_channel,bank_name,bank_btn,money_button_text,telegram_message):
    try:
        async with page.expect_navigation(wait_until="load", timeout=30000):
            ...
            try:
                deposit_submit_button = page.locator('button.btn_deposits.uppercase')
    ...
    if new_payment_page == True:
        ...
        while retry_count < max_retries:
            try:
                ...
                await page.screenshot(path="N9N_%s_%s-%s_Payment_Page.png"%(deposit_method,deposit_channel,bank_name),timeout=30000)
                break 
            except TimeoutError:
...
async def check_toast(page,deposit_method,deposit_channel,bank_name):
    ...
    matches = re.findall(r"रु\s*([\d,]+)", money_input_range_text)
    if matches:
...
async def telegram_send_operation(telegram_message, failed_reason, program_complete):
    ...
    jr_raymond_nepal_chat_id = os.getenv("JR_RAYMOND_NEPAL_CHAT_ID")
    bot = Bot(token=TOKEN)
...
URL: [nep9now\\.com](https://www\\.nep9now\\.com/en\\-np)
TEAM : N9N
...
├─ **TimeOccurred:** `{timestamp}` """ 
            ...
            jr_raymond_caption = f"""[J117711](tg://user?id=235058117), [xiaoq9893](tg://user?id=5504412612)
*Subject: Bot Testing Deposit Gateway*  
URL: [nep9now\\.com](https://www\\.nep9now\\.com/en\\-np)
TEAM : N9N
┌─ **Deposit Testing Result** ──────────┐
...
**Time Detail**  
├─ **TimeOccurred:** `{timestamp}` """
            files = glob.glob("*N9N_%s_%s*.png"%(deposit_method,deposit_channel))
            ...
            if status != 'deposit success':
                ...
                for attempt in range(3):
                    try:
                        with open(file_path, 'rb') as f:
                              await bot.send_photo(
                                    chat_id=jr_raymond_nepal_chat_id,
                                    photo=f,
...
{summary_body}"""
    ...
    for attempt in range(3):
        try:
            await bot.send_message(chat_id=jr_raymond_nepal_chat_id, text=caption, parse_mode='MarkdownV2', disable_web_page_preview=True)
            log.info("SUMMARY SENT")
            break
        except TimedOut:
            log.warning(f"TELEGRAM TIMEOUT，RETRY {attempt + 1}/3...")
...
async def clear_screenshot():
    picture_to_sent = glob.glob("*N9N*.png")
        ...
        try:
            if os.path.exists(file):
                ...
                if "N9N" in sheets:
                    for attempt in range(3):
                        try:
                            df = pd.read_excel(file,sheet_name="N9N")
                        except Exception as e:
                    ...
                    for attempt in range(3):
                        try:
                            with pd.ExcelWriter(
                                ...
                                df.to_excel(writer, sheet_name='N9N', index=False)
                        except Exception as e:
                            log.warning(f"DATA PROCESS EXCEL ERROR: {e}，RETRY {attempt + 1}/3...")
                            await asyncio.sleep(5)
                else:
                    log.info("Sheets N9N not found in file :%s"%file)
